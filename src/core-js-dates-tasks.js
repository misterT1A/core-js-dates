...
function getNextFriday(date) {
  ...
  let result;
  if (newDate.getDay() < 5) {
    result = newDate.setDate(newDate.getDate() + (5 - newDate.getDay()));
  } else {
    result = newDate.setDate(newDate.getDate() + (12 - newDate.getDay()));
  }
...
function getCountWeekendsInMonth(month, year) {
  ...
  let countwek = 0;
  for (let day = start; start < end; day.setDate(day.getDate() + 1)) {
...
function getWeekNumberByDate(date) {
  const startDate = new Date(new Date(date).setMonth(0, 1));
  const firstWeek = 7 - (startDate.getDay() || 7) + 1;

  const time = date - startDate;
  const days = time / (24 * 3600 * 1000) + 1;
  return Math.ceil((days - firstWeek) / 7) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  let friday13 = new Date(year, mounth, 13);
  ...
  while (friday13.getDay() !== 5) {
    ...
    friday13 = new Date(year, mounth, 13);
  }
  return friday13;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  let startDate = new Date(period.start.split('-').reverse().join('-'));
  ...
  while (startDate <= endDate) {
    if (countWork < countWorkDays && isWork) {
      countWork += 1;
      countOff = 0;
      const count = startDate.getDate() + 1;
      const date = `${String(startDate.getDate()).padStart(2, '0')}-${String(
        startDate.getMonth() + 1
      ).padStart(2, '0')}-${startDate.getFullYear()}`;
      result.push(date);
      startDate = new Date(startDate.setDate(count));
    } else {
      isWork = false;
    }
    if (countOff < countOffDays && !isWork) {
      countOff += 1;
      countWork = 0;
      const count = startDate.getDate() + 1;
      startDate = new Date(startDate.setDate(count));
